"#url_shortner_project"
A clean  and simple URL shortener built with django and react ,paste  a long URL and get a short link instantly.

   **Features
- ** url shortening :** Convert long URLs to short codes
- ** Fast Redirects : **Click short link to  go to original URL
- ** SQLite Database : ** Stores all URLs locally
- ** Admin Panel : ** Django admin  to view all links
    **Tech Stack
-**Backend :** Django + Django REST Framework
-**Database : ** SQLite
-**Frontend : ** React - [url_shortner_project_frontend](https://github.com/Aishwarya980/url_shortner_project_frontend.git)
  

  ##Screenshots

  1.Home Page
  
  <img width="1920" height="1080" alt="Screenshot (735)" src="https://github.com/user-attachments/assets/065f6993-eb7f-43b8-b615-1c5b82d8e647" />
  2.Register Page
  <img width="1920" height="1080" alt="Screenshot (735)" src="https://github.com/user-attachments/assets/cedc8c30-3205-48e5-9668-ed9206694db4" />
  3.Main page
  <img width="1920" height="1080" alt="Screenshot (737)" src="https://github.com/user-attachments/assets/0ab15502-09a6-47c5-a9f0-818415ad19a0" />
  <img width="1920" height="1080" alt="Screenshot (738)" src="https://github.com/user-attachments/assets/59d23ce1-f3c2-4c68-8953-5e6e4dd28dff" />
  4.Output page
  <img width="1920" height="1080" alt="Screenshot (739)" src="https://github.com/user-attachments/assets/0b995d18-69bb-4008-88c7-75e5dbd4e5cf" />
  5.Login page
  <img width="1920" height="1080" alt="Screenshot (742)" src="https://github.com/user-attachments/assets/1ab91a62-e89c-4e1c-87d3-7b09eec74b8d" />
  6.Logout page
  <img width="1920" height="1080" alt="Screenshot (741)" src="https://github.com/user-attachments/assets/bdc4a469-03ac-41cc-94d1-a72d359cd4bc" />

  
  
